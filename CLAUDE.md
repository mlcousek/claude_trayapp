...
- `src/ClaudeTrayApp/` WPF app: composition root (`App.xaml.cs`), `Themes/` (`Theme.xaml` tokens, `Controls.xaml` styles, `Palette.Dark.xaml`, `Palette.Light.xaml`), `Theming/` (Windows theme follower), `Tray/` (icon renderer, controller, converter, flyout placement maths), `Views/` (`FlyoutWindow`, `SettingsWindow`), `Controls/` (`RingArc`, `Sparkline`, `LineChart`, `BarChart`: hand-drawn, theme-brushed), `Charts/` (pure chart data builders, loader, palette), `Startup/` (autostart Run entry, single instance, settings coordinator, settings window host), `Assets/` (`app.ico`, generated by `tools/make-icon.py`, our own design), `ViewModels/`, `Hosting/`, `Interop/`, `Diagnostics/` (screenshot aid). The only project that references WPF.
...
- `docs/` `architecture.md`, `data-sources.md`, `packaging.md` (code signing and winget), `diagrams/` (Mermaid sources), `screenshots/`.
